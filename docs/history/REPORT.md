...
| 总代码量 | ~4,500 行 Dart |
| 单元测试 | **40/40** ✅ |
| 代码分析 | **0 error, 0 warning**, 3 info (pre-existing) |
| Flutter SDK | `^3.12.2` |
...
│   ├── record_dao.dart                        # 历史记录 CRUD (插入/批量/分页/解码)
│   ├── draft_dao.dart                         # 草稿 CRUD (固定 id=1 + 过期检查)
│   ├── ai_report_service.dart                 # Dio SSE 流式请求 + [DONE] + CancelToken
│   ├── ai_report_prompt.dart                  # 默认提示词模板 + fillPrompt
│   └── pdf_exporter.dart                      # PDF 生成 (表格 + 图表截图 + 纯文本报告)
│
└── presentation/
    ├── providers/
    ...
    │   └── history_provider.dart              # FutureProvider 历史列表 + 删除
    ├── pages/
...
└── data/
    ├── record_dao_test.dart                   # 7 测试 (CRUD + 分页 + 批量)
...
| file_picker 导入导出 | 待实现 | Phase 5 |
| AiConfig 加密存储 | 待实现 | Phase 5 |
...
$ flutter test
  00:01 +40: All tests passed! # 27 engine + 7 RecordDao + 6 DraftDao
```
...
| 中文字体 | `assets/fonts/NotoSansSC-Regular.otf` | fonttools 子集化 ~1–2MB |
...
- **字体子集化**: `fonttools` 提取 3500 常用字, 原始 `NotoSansSC-Regular.otf` 约 8–10MB, 子集化后约 1–2MB
...
| 字体子集化 (fonttools) | 暂未实现 | PDF 使用内置字体，中文可能显示为方框，Phase 6 解决 |

> **字体子集化补丁** (Phase 6): 获取 `NotoSansSC-Regular.otf` → `fonttools subset --text-file=cjk-3500.txt --output=assets/fonts/NotoSansSC-Regular.subset.otf` → 注册 `Font.ttf()`。当前 PDF 中文显示取决于系统是否提供中文字体。

---

## Phase 5 预告 — 设置 + 对比 + 打磨

- 设置页面: AiConfig 双源拼接 (SharedPreferences + flutter_secure_storage)
- 多记录对比页面 (按 ID 按需加载)
- JSON 导入/导出 (file_picker)
- UI 打磨: 响应式布局、动画、离线状态
- 端到端测试
